  ...
  <header class="bg-gray-50 py-10 sm:py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center">
        ...
        After you've checked out all the content on the page, and if you are willing, please click the '<a href="signup.php" class="block px-4 py-2 text-gray-600 hover:text-gray-900">Create account</a></li>' button to sign up."
        </p>
        <div class="mt-8">
 ...
 <!-- Concert Highlights Section -->
    <section class="mb-12">
	  ...
	  <div class="fixed bottom-6 right-6">
       <h2><a href="events.php" class="bg-blue-500 hover:bg-indigo-700 text-white py-3 px-6 rounded-full shadow-lg font-semibold transition duration-300">
        More Events
      </a></h2>
    </div>
    </section>
...
<section class="bg-gray-100 py-12 px-6 rounded-lg">
  <div class="max-w-md mx-auto">
    <h2 class="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-800 mb-6">Let us help you host your event</h2>
    <?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $event_host_name = $_POST['event_host_name'];
        $title = $_POST['title'];
        $description = $_POST['description'];
        $phone_number = $_POST['phone_number'];

        // Validate inputs
        if (empty($event_host_name) || empty($title) || empty($description) || empty($phone_number)) {
            $message = 'Please fill in all the fields.';
        } elseif (!ctype_digit($phone_number)) {
            $message = 'Please enter a valid phone number with only digits.';
        } else {
            // Insert data into the events_request table
            $stmt = $conn->prepare("INSERT INTO events_request (event_host_name, title, description, phone_number) VALUES (:event_host_name, :title, :description, :phone_number)");
            $stmt->bindValue(':event_host_name', $event_host_name);
    ...
    if (!empty($message)) {
        $color = ($message === 'Form submitted successfully!') ? 'text-green-500' : 'text-red-500';
        echo '<p class="' . $color . ' mb-4">' . $message . '</p>';
    }
    ?>
    <form method="post">
      <div class="mb-4">
        <label for="event_host_name" class="block text-sm font-semibold text-gray-600">Event Host Name</label>
        <input type="text" id="event_host_name" name="event_host_name" class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500" placeholder="Enter your name" required>
      </div>
      <div class="mb-4">
        <label for="title" class="block text-sm font-semibold text-gray-600">Title</label>
        <input type="text" id="title" name="title" class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500" placeholder="Enter event title" required>
      </div>
      <div class="mb-4">
        <label for="description" class="block text-sm font-semibold text-gray-600">Description</label>
        <textarea id="description" name="description" rows="4" class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500" placeholder="Enter event description" required></textarea>
      </div>
      <div class="mb-4">
        <label for="phone_number" class="block text-sm font-semibold text-gray-600">Phone Number</label>
        <input type="tel" id="phone_number" name="phone_number" class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500" placeholder="Enter phone number" required>
      </div>
      <button type="submit" class="inline-block w-full bg-blue-500 text-white px-4 py-3 rounded-md font-semibold text-center hover:bg-blue-600 transition duration-300">Submit</button>
    </form>
  </div>
</section>
